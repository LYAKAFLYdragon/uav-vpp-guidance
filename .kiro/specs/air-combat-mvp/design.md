...
**动作空间**：第一阶段默认沿用父类动作空间（VPP 偏移或直接指令的 3 维），**导弹发射由规则触发，不占用动作维度**（需求 3.6）。

> **第二阶段扩展（缺陷 2 修复，可选）**：当 `air_combat.action.use_launch_action=true` 时，动作空间扩展为 **4 维** `[VPP 偏移 x, y, z, 发射决策]`。前 3 维仍作 VPP 偏移透传父类；第 4 维为连续值，经阈值 `launch_action_threshold`（默认 0.0）二值化为"策略是否请求发射"。**实际发射 = 物理可行（锁定 AND 无在飞弹 AND can_launch）AND 策略请求发射**——即策略学习"何时发射"的战术决策（如 BVR 占位后发射 vs WVR 格斗发射），但不能违反物理约束发射。环境通过 `action_dim` 属性暴露当前维度（4 或 3），训练脚本据此配置策略网络输出维度。可选的发射时机奖励塑形 `launch_shaping_reward`（默认 0）对"可行窗口内请求发射"给予小幅正奖励、对"可行窗口内放弃发射"给予小幅负奖励，缓解发射决策稀疏性；默认关闭以保持纯稀疏语义。该扩展默认关闭，与第一阶段规则发射行为完全向后兼容。
