...
|------|------|-------------|----------------|
| **favorable** | Ego 在目标后方，速度更快 | 位置 `[0,0,5000]`，航向 `0°`，速度 `280 m/s` | 位置 `[800,0,5000]`，航向 `0°`，速度 `180 m/s` |
| **neutral** | 迎头相遇，速度相近 | 位置 `[0,0,5000]`，航向 `0°`，速度 `200 m/s` | 位置 `[2000,0,5000]`，航向 `180°`，速度 `200 m/s` |
| **disadvantage** | Target 在 Ego 侧后方且更快 | 位置 `[0,0,5000]`，航向 `0°`，速度 `200 m/s` | 位置 `[-600,400,5000]`，航向 `30°`，速度 `220 m/s` |
