...
- [ ] **当前唯一允许动作：** 等待用户对该单一 neutral-post-merge pilot 作明确的一次性执行授权。授权后才可创建独立 authorization config，并在运行前再次检查 clean worktree、代码/资产 SHA、空 output root、raw-SI phase、ATA/AA 与 stop rule。不得重跑或修补 B6，也不得直接训练四共享技能、扩成高层 PPO 或启动 formal held-out。
