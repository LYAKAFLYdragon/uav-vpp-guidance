...
- [ ] **当前唯一允许动作：按已授权 config 一次性执行 B4 raw-SI phase preflight，随后立即关闭执行开关并归档。** 设计见 `reports/thesis_global_advantage_p2_b4_raw_si_phase_design_20260715_zh.md`：新 Source ID、非重合场景/seed，从 `observation.relative_state` 记录原始米/米每秒 phase 输入，同时保留 normalized policy observation 仅作诊断；不得修改或重跑 B3，不得训练、调参或启动 formal held-out。
