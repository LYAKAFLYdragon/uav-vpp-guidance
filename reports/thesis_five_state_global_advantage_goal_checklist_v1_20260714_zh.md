...
- [ ] **当前唯一允许动作：在 B3 implementation freeze 后独立复核并一次性执行新的 P2-B3 phase-observability preflight。** 预注册见 `reports/thesis_global_advantage_p2_b3_phase_observability_design_20260715_zh.md`：新 Source ID 和不重合场景/seed，从 reset 与 step 0 起记录 base observation、range/range-rate、PhaseTracker input/output、first-pass 和 scenario-application receipt；不得补跑或覆盖 P2-B2，不得直接训练、调参或启动 formal held-out。
