...
- [x] **Neutral-post-merge pilot 设计预检已闭合：** 已实现并验证独立 `train/dev12/heldout24` continuous manifests、runtime registry、artifact schema、contract tests 与 fail-closed preflight。两份 manifest 对 B2--B6、`heldout240`、`dev30`、旧 `heldout60` 与 phase-v2 的物理签名及 seed 均为零交集；预检确认冻结 P3、两基线和两个神经 opponent 的 SHA，且 `execution_permitted=false`、`training_permitted=false`。见 `reports/thesis_neutral_postmerge_reentry_recovery_pilot_design_preflight_20260715_zh.md`。该完成不构成 pilot 结果或执行授权。
- [x] **Neutral-post-merge pilot 独立复核已通过：** 在 clean `bc356bc5165906618c83191252cd9142e52cb4a2` 上复核 manifest 再生、payload hash、资产 SHA、ATA/AA、raw-SI phase、66-D/3-D action、output capacity、stop rule 与测试；`10 passed`，且所有 execution/training permission 仍为 false。见 `reports/thesis_neutral_postmerge_reentry_recovery_pilot_independent_review_20260715_zh.md`。该完成仍不构成 pilot 结果或执行授权。
- [x] **授权前物理契约修正：** execution runner 的 sampler test 在未创建任何 JSBSim pilot output 前发现 `172 deg` neutral support 与最大高度差不兼容；现已收紧为 `165 deg`、加入显式可行性 fail-closed 检查，并补齐遗漏的第三 opponent `+0.02` 非劣门槛。见 `reports/thesis_neutral_postmerge_reentry_recovery_pilot_execution_preflight_correction_20260715_zh.md`。修正后的设计预检和合同测试通过，但旧独立复核不再是最终执行复核。
- [x] **Neutral-post-merge pilot V1 一次性执行已关闭：** 授权 preflight 后 strict JSBSim 启动，但首个 dev baseline 在写 telemetry ledger 时因 `metadata.scenario_signature` 不存在而触发 `KeyError`；output root 无 checkpoint、dev/heldout record 或逐步 telemetry，故固定为 `implementation_failure_not_experimental_evidence`，不产生性能、安全或技能库结论。详见 `reports/thesis_neutral_postmerge_reentry_recovery_pilot_v1_implementation_failure_20260715_zh.md`。同一 Source ID 禁止修补或重跑。
- [x] **Neutral-post-merge pilot V2 已执行并关闭：** design preflight 与 adapter tests 通过后，strict JSBSim 已写入 72 条 frozen dev baseline；但第一条 train scenario 把 evaluation `pair_key` 强加于未携带 `geometry_cell_id` 的 train identity 而失败。无 checkpoint、candidate record 或 heldout，故固定为 `implementation_failure_not_experimental_evidence`。同时 baseline handoff 为 expert `1/12`、end-to-end `0/12`、independent PPO/VPP `1/12`，不能支持 V2 的后续门槛。见 `reports/thesis_neutral_postmerge_reentry_recovery_pilot_v2_implementation_failure_20260715_zh.md`；V2 禁止修补或重跑。
- [x] **V3 runtime-feasibility design 与独立静态复核已完成：** `THESIS-NEUTRAL-POSTMERGE-REENTRY-RECOVERY-RUNTIME-FEASIBILITY-V3` 固定了 48 个物理不相交的 neutral/pre-merge family、evaluation/train 互斥身份、三 opponent 独立 8-episode/160-step/2-mirror/3-height gate、future pilot geometry reservation 与输出容量；future runner 还冻结了 reset receipt、raw-SI phase replay、handoff/66-D/3-D VPP telemetry、V3 provenance、完整 144-record universe 与 overlay scope guard。manifest 可字节再生、`11 passed`、design preflight 通过、未创建 output root、无 overlay 的 `--execute` 被拒绝。见 `reports/thesis_neutral_postmerge_reentry_recovery_v3_runtime_feasibility_design_20260715_zh.md` 与 `reports/thesis_neutral_postmerge_reentry_recovery_v3_runtime_feasibility_independent_review_20260715_zh.md`。这不构成 JSBSim 结果或训练授权。
- [ ] **当前唯一允许动作：** 申请并审查单独的一次性 V3 runtime-feasibility 执行授权。授权 config 必须哈希绑定 V3 canonical design、clean implementation SHA、code whitelist、144-record fresh output root 与三 opponent stop rule；不得重跑或修补 V1/V2/B6，也不得训练四共享技能、扩成高层 PPO 或启动 formal held-out。
- [ ] P2--P8 均未授权；尤其不允许以“追求全场景优势”为由绕过既有负证据、V3 runtime gate 或 stop rule。
