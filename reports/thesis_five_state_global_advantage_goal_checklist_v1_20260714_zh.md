...
- [x] **P2-B5 设计与实现：** 已建立 `THESIS-GLOBAL-ADVANTAGE-V1-P2-PHASE-FEASIBLE-SAMPLER-B5-R1` 的独立 12 场景 manifest、raw-SI collector、fail-closed runner/preflight 与 tests；设计预检通过，输出根为空，`execution_permitted=false`。详见 `reports/thesis_global_advantage_p2_b5_phase_feasible_sampler_preregistration_20260715_zh.md`。
- [x] **P2-B5 独立复核：** 已补齐连续 handoff/真实 66-D 路径测试，并修正 dynamic taxonomy 的 `(ATA, AA)` 参数顺序；当前 `14 passed`，不涉及 JSBSim 实验结果。
- [ ] **当前唯一允许动作：执行一次 P2-B5 已授权 sampler。** 仅可使用 `reports/thesis_global_advantage_p2_b5_phase_feasible_sampler_execution_authorization_20260715_zh.md` 中的唯一命令；执行后关闭授权并按三 opponent gate 判定，仍不得训练、调参或启动 formal held-out。
