...
- [ ] **当前唯一允许动作：设计并独立复核 P2-B4 raw-SI phase preflight。** 它必须采用新 Source ID、非重合场景/seed，并从 `observation.relative_state` 记录原始米/米每秒 phase 输入，同时保留 normalized policy observation 仅作诊断；不得修改或重跑 B3，不得训练、调参或启动 formal held-out。
