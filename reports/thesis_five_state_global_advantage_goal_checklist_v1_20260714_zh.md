...
- [ ] **当前唯一允许动作：** 若继续 neutral/post-merge 机制，先设计新的 V2 Source ID，修复 metadata pairing contract 并对真实 manifest 添加 round-trip 测试；V2 必须使用新 output root、重新预注册与单独授权。不得重跑或修补 B6，也不得训练四共享技能、扩成高层 PPO 或启动 formal held-out。
