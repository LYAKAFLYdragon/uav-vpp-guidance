...
- [x] **Neutral-post-merge pilot V2 已预注册：** 新 dev12/heldout24 对 V1 和所有既有冻结来源零物理初态/seed 交集；`geometry_cell_id + scenario_seed` 已冻结为序列化 `pair_key`，真实 manifest round-trip test 与 design preflight 通过，P3/runtime registry 也已哈希绑定。见 `reports/thesis_neutral_postmerge_reentry_recovery_pilot_v2_preregistration_20260715_zh.md`。V2 未运行 JSBSim，未继承 V1 执行授权。
- [x] **Neutral-post-merge pilot V2 独立复核：** V2 adapter 无直接 `metadata.scenario_signature` 读取，真实 pair-key 篡改会 fail-closed，scoped V1 engine override 在退出后完全恢复；design preflight 和 adapter tests 通过。见 `reports/thesis_neutral_postmerge_reentry_recovery_pilot_v2_independent_review_20260715_zh.md`。V2 未运行 JSBSim，未继承 V1 执行授权。
- [ ] **当前唯一允许动作：** 将 V2 wrapper 和设计文件固定为新的 clean SHA 后，由用户单独授权一次性执行。授权前不得创建 V2 output root、训练或运行 JSBSim；不得重跑或修补 B6，也不得训练四共享技能、扩成高层 PPO 或启动 formal held-out。
