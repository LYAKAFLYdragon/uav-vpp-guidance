...
- [ ] **当前唯一允许动作：按已授权的 P2-B2 config 在冻结实现上一次性执行 strict-JSBSim physical-reachability preflight，随后立即关闭执行开关并归档。R5 capability card 的 phase 仅为 post_merge，不能代替此 gate。不得直接训练、调参或启动 formal held-out。**
