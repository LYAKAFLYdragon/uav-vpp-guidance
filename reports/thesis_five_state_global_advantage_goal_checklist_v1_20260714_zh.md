...
- [ ] **当前唯一允许动作：设计并独立复核 P2-B5 phase-feasible sampler。** 它必须用连续 run-in 到 handoff、按 state×opponent 明确收集真实 raw-SI phase receipt，并把 `disadvantage -> post_merge/re_entry` 在三 opponent 的缺口作为首要可证伪对象；不得直接训练、调参或启动 formal held-out。
