...
- [ ] **当前唯一允许动作：** 将 neutral-post-merge pilot 设计文件固定为新的 clean SHA，并对其 manifest、ATA/AA、raw-SI phase、资产 hash、输出容量和 stop rule 做独立复核；仅在单独的一次性授权后才可执行该单技能 pilot。不得重跑或修补 B6，也不得直接训练四共享技能、扩成高层 PPO 或启动 formal held-out。
