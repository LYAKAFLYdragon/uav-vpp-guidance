...
- [ ] **当前唯一允许动作（用户已授权，执行前置仍未完成）：** 将修正后的 runner、runtime asset registry 和设计 config 固定为新的 clean SHA，生成并独立复核一次性 authorization config；随后才可执行该单一 pilot。不得重跑或修补 B6，也不得训练四共享技能、扩成高层 PPO 或启动 formal held-out。
