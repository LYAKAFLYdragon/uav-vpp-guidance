...
- [ ] **当前唯一允许动作：独立复核 P2-B5 预注册，随后才可决定是否一次性授权。** B5 必须以连续 run-in 到 handoff、按 opponent 分开的 raw-SI phase receipt，证伪或建立 `disadvantage -> post_merge/re_entry` 的 66-D 数据契约；不得直接训练、调参或启动 formal held-out。
