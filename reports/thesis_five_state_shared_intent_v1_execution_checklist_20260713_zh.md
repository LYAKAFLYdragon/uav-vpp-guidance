...
- [x] `THESIS-NEUTRAL-POSTMERGE-REENTRY-RECOVERY-PILOT-V2` 已执行并冻结为实现失败，不构成实验正/负证据。授权 preflight 和 72 条 frozen dev baseline telemetry 已完成，但 train sampler 没有 V2 ledger 强制要求的 `geometry_cell_id + pair_key`，在第一条 train scenario 的序列化阶段失败；无 checkpoint、candidate record 或 heldout。已保存 baseline 还显示 `dev12` 的 handoff 覆盖为 expert `1/12`、end-to-end `0/12`、independent PPO/VPP `1/12`，不能支持三对手最小 2 条或 claim-ready 8 条门槛。详见 `reports/thesis_neutral_postmerge_reentry_recovery_pilot_v2_implementation_failure_20260715_zh.md`。不得重跑 V2；任何 V3 必须先独立设计 train identity contract 与逐 opponent runtime phase-feasibility preflight，四共享技能、combat finetune 与 P5/P6/P7 继续锁定。
- [x] `THESIS-NEUTRAL-POSTMERGE-REENTRY-RECOVERY-RUNTIME-FEASIBILITY-V3` 已完成 design-only 固化与独立静态复核：48 个与 V1/V2/B2--B6 物理不相交的 neutral/pre-merge 场景组成 4 个 family；evaluation `pair_key` 与 train `train_stream_id` 已在代码和测试中互斥；每个 family 必须在 expert、end-to-end、independent PPO/VPP 下各自满足 `12` record、`>=8` qualifying episode、`>=160` valid step、2 mirror、3 height 和零 telemetry contract failure 才可成为未来 pilot 输入候选。future runner 已实现 reset receipt、raw-SI phase replay、handoff/66-D/3-D VPP telemetry、V3 provenance、完整 144-record universe 与授权 overlay 范围保护；当前 `11 passed`、design preflight 全绿、V3 output root 未创建、无 overlay 的 `--execute` 被拒绝。详见 `reports/thesis_neutral_postmerge_reentry_recovery_v3_runtime_feasibility_design_20260715_zh.md` 和 `reports/thesis_neutral_postmerge_reentry_recovery_v3_runtime_feasibility_independent_review_20260715_zh.md`。这不是 V3 运行结果，不解锁任何训练；唯一下一步是申请一次性 runtime-feasibility 执行授权。
