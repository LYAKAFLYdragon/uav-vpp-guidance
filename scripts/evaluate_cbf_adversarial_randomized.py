#!/usr/bin/env python3
"""
Evaluate a pursuer-target pair in AdversarialJSBSimEnv with randomized initial conditions.

This script extends evaluate_cbf_adversarial.py by adding random perturbations to the
scenario initial conditions (range, heading, speed, altitude). This breaks the
determinism of repeated episodes and provides non-zero variance for statistical
analysis and paper-grade error bars.
...
def load_perturbation_config(config_path: Optional[str]) -> Dict[str, Any]:
    """Load random-perturbation parameters. Returns empty dict if no config."""
    if config_path is None:
        return {"enabled": False}
    cfg = load_yaml_config(config_path)
    return cfg.get("perturbation", {"enabled": True})


def _perturb_aircraft_init(
    init: Dict[str, Any], rng: np.random.Generator, base_range: float, cfg: Dict[str, Any]
) -> Dict[str, Any]:
    """Apply bounded random perturbations to a single aircraft init dict."""
    init = copy.deepcopy(init)
    pos = np.asarray(init.get("position_m", [0.0, 0.0, 5000.0]), dtype=float).copy()
    speed = float(init.get("velocity_mps", 200.0))
    heading = float(init.get("heading_deg", 0.0))

    rel_xy = cfg.get("position_xy_relative", 0.10)
    rel_alt = cfg.get("altitude_relative", 0.05)
    rel_speed = cfg.get("speed_relative", 0.10)
    heading_deg = cfg.get("heading_deg", 5.0)

    # Horizontal position: move up to rel_xy * base_range in a random direction.
    max_delta_xy = rel_xy * max(base_range, 1.0)
    delta_xy = rng.uniform(0.0, max_delta_xy)
    angle = rng.uniform(0.0, 2.0 * math.pi)
    pos[0] += delta_xy * math.cos(angle)
    pos[1] += delta_xy * math.sin(angle)

    # Altitude.
    pos[2] *= 1.0 + rng.uniform(-rel_alt, rel_alt)
    pos[2] = float(np.clip(pos[2], 600.0, 14000.0))

    # Speed.
    speed *= 1.0 + rng.uniform(-rel_speed, rel_speed)
    speed = float(np.clip(speed, 50.0, 400.0))

    # Heading.
    heading += rng.uniform(-heading_deg, heading_deg)

    init["position_m"] = pos.tolist()
    init["velocity_mps"] = speed
    init["heading_deg"] = heading
    return init


def perturb_scenario(
    scenario: Dict[str, Any], rng: np.random.Generator, cfg: Dict[str, Any]
) -> Dict[str, Any]:
    """Apply random perturbations to both own and target initial conditions."""
    if not cfg.get("enabled", False):
        return scenario

    scenario = copy.deepcopy(scenario)
    own = scenario.get("own_init", scenario.get("pursuer_init", {}))
    target = scenario.get("target_init", scenario.get("target", {}))

    own_pos = np.asarray(own.get("position_m", [0.0, 0.0, 5000.0]), dtype=float)
    target_pos = np.asarray(target.get("position_m", [0.0, 0.0, 5000.0]), dtype=float)
    base_range = float(np.linalg.norm(own_pos - target_pos))

    scenario["own_init"] = _perturb_aircraft_init(own, rng, base_range, cfg)
    scenario["target_init"] = _perturb_aircraft_init(target, rng, base_range, cfg)
    return scenario
...
def run_adversarial_episode(
    ...
    """Run one adversarial episode with optional initial-condition perturbation."""
    rng = np.random.default_rng(seed)
    if isinstance(scenario, dict):
        scenario = perturb_scenario(scenario, rng, perturbation_cfg)
...
def main() -> None:
    ...
    parser.add_argument("--perturbation-config", type=str, default="config/eval/perturbation_default.yaml")
    parser.add_argument("--output-dir", type=str, default=None)
    args = parser.parse_args()

    config = load_experiment_config(args.config)
    perturbation_cfg = load_perturbation_config(args.perturbation_config)
