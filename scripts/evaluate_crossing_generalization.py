#!/usr/bin/env python3
"""
Crossing Geometry Generalization Test.

Evaluate constrained and baseline models on a diverse set of crossing geometries:
  - Initial distance: 1000, 1500, 2000, 2500, 3000 m
  - Crossing angle: 90, 135, 180, 225, 270 deg (relative to ego heading=0)
  - Speed ratio: 0.8, 1.0, 1.2
  - Lateral offset: ±1000, ±1500, ±2000 m
"""
import sys, os, json
...
def make_crossing_scenario(range_m, crossing_angle_deg, speed_ratio, lateral_offset_m):
    """Create a crossing scenario.

    Ego at origin, heading 0° (along +x).
    Target placed at (range_m * cos(theta), range_m * sin(theta))
    where theta accounts for lateral offset.

    Target heading = crossing_angle_deg (direction of target movement).
    """
    target_speed = 200.0
    ego_speed = target_speed * speed_ratio

    # Place target such that lateral offset is lateral_offset_m
    # and range is approximately range_m
    if lateral_offset_m == 0:
        target_x = range_m
        target_y = 0.0
    else:
        # Target at (x, y) with sqrt(x^2 + y^2) ≈ range_m and |y| = lateral_offset
...
def evaluate_on_scenarios(env, agent, scenarios, num_episodes=10):
    results = {}
...
def main():
    ...
    for method_name, cfg, ckpt_template in [
        ...
        for seed in [0, 1, 2]:
            ...
            agent.load(ckpt)
            results = evaluate_on_scenarios(env, agent, scenarios, num_episodes=5)
    ...
    with open("docs/results/crossing_generalization/summary.md", "w") as f:
        f.write("# Crossing Geometry Generalization\n\n")
        f.write(f"Total scenarios: {len(scenarios)} (5 ranges × 5 angles × 3 speeds × 7 offsets)\n\n")
        ...
        f.write("\n## Evidence Grade\n")
        f.write("`preliminary` — 3 seeds, 5 episodes per scenario, 175 scenarios.\n")
        f.write("Requires 10 seeds for `paper_safe`.\n")

    with open("docs/results/crossing_generalization/raw_results.json", "w") as f:
