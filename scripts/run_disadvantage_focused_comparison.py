#!/usr/bin/env python3
"""Compare baseline VPP vs. disadvantage-focused VPP on maneuvering target.

Usage:
    python scripts/run_disadvantage_focused_comparison.py --timesteps 30000 --episodes 20
"""
...
def train_method(key: str, output_dir: str, device: str, timesteps: int) -> Path:
...
def main():
    ...
    parser.add_argument("--timesteps", type=int, default=30000, help="Override ppo.total_timesteps")
    ...
    for key, info in METHODS.items():
        exp_name = f"maneuver_target_{key}_vpp_30k_s0"
        ...
        if not args.skip_train and (args.retrain or not checkpoint.exists()):
            train_method(key, output_dir, args.device, args.timesteps)
        else:
    ...
    print("\n" + "=" * 100)
    print(f"{'Method':<28} {'Scenario':<18} {'N':>4} {'Success%':>10} {'Crash%':>8} {'Timeout%':>10} {'MeanReturn':>12} {'MeanMinRng':>12} {'MeanFinRng':>12}")
    print("-" * 100)
    for key, res in results.items():
        for scen, stats in res["per_scenario"].items():
            print(
                f"{res['method']:<28} {scen:<18} {stats['n']:>4} "
        ...
        ov = res["overall"]
        print(
            f"{res['method']:<28} {'OVERALL':<18} {ov['n']:>4} "
    ...
    out_file = out_dir / "disadvantage_focused_comparison.json"
