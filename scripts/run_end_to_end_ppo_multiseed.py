#!/usr/bin/env python3
"""
Multi-seed PPO training launcher for the end-to-end baseline.

Runs train_end_to_end_ppo across N seeds (default 3) and produces a summary
report compatible with the hierarchical baseline evaluation pipeline.

Usage:
    # Standard 3-seed run (200K steps each)
    python scripts/run_end_to_end_ppo_multiseed.py \
    ...
    python scripts/run_end_to_end_ppo_multiseed.py ... --skip-existing
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Multi-seed end-to-end PPO training"
    )
    parser.add_argument(
        "--config",
        type=str,
        required=True,
        help="Path to experiment config YAML (e.g. train_end_to_end_ppo.yaml).",
    )
    parser.add_argument(
        "--seeds",
        type=int,
...
def _load_yaml_name(config_path: str) -> str:
    ...
    # Remove common training prefix but preserve the baseline identifier
