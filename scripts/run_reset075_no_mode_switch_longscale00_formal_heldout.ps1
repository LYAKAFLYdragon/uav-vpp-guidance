...
$expertArgs = $commonArgs + @(
    "--run-id", "reset075_no_mode_switch_longscale00_formal_heldout_expert",
    "--opponent-stage", "expert"
)

$endToEndArgs = $commonArgs + @(
    "--run-id", "reset075_no_mode_switch_longscale00_formal_heldout_end_to_end",
    "--opponent-stage", "end_to_end"
)

Write-Host "Running reset075 no-mode-switch longscale00 held-out formal evaluation against expert..."
& python @expertArgs

Write-Host "Running reset075 no-mode-switch longscale00 held-out formal evaluation against end_to_end..."
& python @endToEndArgs
