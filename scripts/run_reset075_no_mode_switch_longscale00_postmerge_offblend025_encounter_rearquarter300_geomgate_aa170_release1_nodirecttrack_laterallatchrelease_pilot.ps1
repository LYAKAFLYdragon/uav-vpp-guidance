...
$runLabel = "laterallatch_release"
$runSuffix = ""
if ($LateralOnlyHoldSteps -ge 0) {
    $vppArgs += @(
        "--vpp-post-merge-offensive-anchor-blend-release-lateral-only-hold-steps-by-task",
        "head_on=$LateralOnlyHoldSteps,crossing_feasible=0"
    )
    $runLabel = "laterallatch_release_hold$LateralOnlyHoldSteps"
    $runSuffix = "_hold$LateralOnlyHoldSteps"
}
...
$candidateRunTag = "reset075_no_mode_switch_longscale00_postmerge_offblend025_encounter_rearquarter300_geomgate_aa170_release1_nodirecttrack_laterallatchrelease$($runSuffix)_pilot"
...
Write-Host "Running lateral-latch release rear-quarter pilot against expert..."
& $PythonExe @expertArgs

Write-Host "Running lateral-latch release rear-quarter pilot against end_to_end..."
& $PythonExe @endToEndArgs

$analysisOutputDir = "E:\uav-vpp-guidance\outputs\diagnostics\rearquarter_laterallatchrelease$($runSuffix)_pilot"
...
Write-Host "Building lateral-latch release rear-quarter pilot comparison report..."
