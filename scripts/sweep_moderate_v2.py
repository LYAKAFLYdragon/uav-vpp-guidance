#!/usr/bin/env python3
"""Quick geometry sweep for the moderate v2 scene."""
...
def base_config(name: str, geo: dict) -> str:
    return f"""# Auto-generated moderate sweep config
includes:
...
def main():
    candidates = [
        # (own_v, tgt_v, tgt_h, tgt_x, tgt_y)
        (200.0, 210.0, 330.0, 700.0, 1000.0),  # current retune
        (195.0, 215.0, 325.0, 600.0, 1050.0),
        (190.0, 215.0, 330.0, 700.0, 1000.0),
        (185.0, 220.0, 325.0, 600.0, 1050.0),
        (180.0, 225.0, 320.0, 500.0, 1000.0),
        (180.0, 230.0, 340.0, 300.0, 1200.0),
        (175.0, 230.0, 335.0, 400.0, 1150.0),
        (180.0, 220.0, 335.0, 500.0, 1100.0),
        (185.0, 215.0, 335.0, 550.0, 1050.0),
    ]
    ...
    for i, (o_v, t_v, t_h, x, y) in enumerate(candidates):
        geo = {
            ...
            "tgt_x": x, "tgt_y": y,
            "description": f"Sweep candidate {i}: ego {o_v:.0f}/tgt {t_v:.0f}/h{t_h:.0f}",
        }
        rng, ata, aa, closure = geometry_metrics(geo)
        cfg_name = f"_tmp_moderate_sweep_{i:02d}"
        ...
        print(f"[{i}] ego={o_v:.0f} tgt={t_v:.0f} h={t_h:.0f} pos=[{x:.0f},{y:.0f}] "
              f"rng={rng:.0f} ATA={ata:.0f} AA={aa:.0f} closure={closure:.0f}")
    ...
    out_path = _PROJECT_ROOT / "outputs" / "disadvantage_v2" / "moderate_sweep.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(results, indent=2), encoding="utf-8")
    print(f"\nSaved sweep results to {out_path}")

    # Print sorted by train_sr
    print("\nSorted by train SR:")
    for r in sorted(results, key=lambda x: (x["train_sr"] is None, x["train_sr"] or -1)):
        print(r)
