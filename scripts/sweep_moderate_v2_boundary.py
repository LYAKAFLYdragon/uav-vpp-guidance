#!/usr/bin/env python3
"""Boundary sweep for moderate v2 scene."""
...
def base_config(name: str, geo: dict) -> str:
    return f"""# Auto-generated moderate boundary sweep config
includes:
...
def main():
    candidates = [
        # Boundary between 100% (candidate 7) and 0% (candidate 6)
        (180.0, 220.0, 335.0, 500.0, 1100.0),
        (179.0, 222.0, 335.0, 480.0, 1110.0),
        (178.0, 224.0, 335.0, 460.0, 1120.0),
        (177.0, 225.0, 335.0, 450.0, 1125.0),
        (176.0, 226.0, 335.0, 440.0, 1130.0),
        (175.0, 227.0, 335.0, 430.0, 1140.0),
    ...
    for i, (o_v, t_v, t_h, x, y) in enumerate(candidates):
        geo = {
            ...
            "tgt_x": x, "tgt_y": y,
            "description": f"Boundary candidate {i}: ego {o_v:.0f}/tgt {t_v:.0f}/h{t_h:.0f}",
        }
        rng, ata, aa, closure = geometry_metrics(geo)
        cfg_name = f"_tmp_moderate_boundary_{i:02d}"
        ...
        print(f"[{i}] ego={o_v:.0f} tgt={t_v:.0f} h={t_h:.0f} pos=[{x:.0f},{y:.0f}] "
              f"rng={rng:.0f} ATA={ata:.1f} AA={aa:.0f} closure={closure:.1f}")
    ...
    out_path = _PROJECT_ROOT / "outputs" / "disadvantage_v2" / "moderate_boundary_sweep.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(results, indent=2), encoding="utf-8")
    print(f"\nSaved boundary sweep results to {out_path}")
