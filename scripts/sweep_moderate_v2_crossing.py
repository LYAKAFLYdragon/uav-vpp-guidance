#!/usr/bin/env python3
"""Crossing-geometry sweep for moderate v2 scene."""
...
def base_config(name: str, geo: dict) -> str:
    return f"""# Auto-generated moderate crossing sweep config
includes:
...
def main():
    candidates = [
        (190.0, 220.0, 310.0, 700.0, 1000.0),
        (185.0, 220.0, 310.0, 600.0, 1050.0),
        (180.0, 220.0, 310.0, 500.0, 1100.0),
        (190.0, 225.0, 305.0, 600.0, 1000.0),
        (185.0, 225.0, 305.0, 500.0, 1050.0),
        (180.0, 225.0, 300.0, 450.0, 1100.0),
        (185.0, 230.0, 300.0, 400.0, 1050.0),
        (180.0, 230.0, 295.0, 350.0, 1100.0),
        (175.0, 235.0, 290.0, 250.0, 1150.0),
    ]
    ...
    for i, (o_v, t_v, t_h, x, y) in enumerate(candidates):
        geo = {
            ...
            "tgt_x": x, "tgt_y": y,
            "description": f"Crossing candidate {i}: ego {o_v:.0f}/tgt {t_v:.0f}/h{t_h:.0f}",
        }
        rng, ata, aa, closure = geometry_metrics(geo)
        cfg_name = f"_tmp_moderate_cross_{i:02d}"
    ...
    out_path = _PROJECT_ROOT / "outputs" / "disadvantage_v2" / "moderate_crossing_sweep.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(results, indent=2), encoding="utf-8")
    print(f"\nSaved crossing sweep results to {out_path}")
