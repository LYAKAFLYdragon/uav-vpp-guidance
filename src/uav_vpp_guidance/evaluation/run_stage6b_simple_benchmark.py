...
def evaluate_single_episode(
    ...
    # Terminal-phase stability tracking
    step_commands = []
    step_ranges = []
    ...
    for step in range(env.max_steps):
        ...
        # Record command and range for terminal-phase stability analysis
    ...
    # Compute terminal-phase stability metrics
    terminal_metrics = _compute_terminal_metrics(step_commands, step_ranges, config)
...
def _compute_terminal_metrics(step_commands, step_ranges, config):
    """Compute terminal-phase stability metrics from per-step command history."""
    if not step_commands:
        return {
            "terminal_nz_cmd_variance": np.nan,
            "terminal_roll_rate_cmd_variance": np.nan,
            "terminal_throttle_cmd_variance": np.nan,
    ...
    n = len(step_commands)
    terminal_start = max(0, n - max(5, int(np.ceil(0.2 * n))))
    terminal_cmds = step_commands[terminal_start:]
    terminal_ranges = step_ranges[terminal_start:]

    nz_vals = [c["nz_cmd"] for c in terminal_cmds if np.isfinite(c["nz_cmd"])]
    roll_vals = [
        c["roll_rate_cmd"] for c in terminal_cmds if np.isfinite(c["roll_rate_cmd"])
    ]
    throttle_vals = [
        c["throttle_cmd"] for c in terminal_cmds if np.isfinite(c["throttle_cmd"])
    ]
    ...
    nz_exceed = sum(1 for v in nz_vals if v < nz_min or v > nz_max) / max(
        1, len(nz_vals)
    )
    rr_exceed = sum(1 for v in roll_vals if v < rr_min or v > rr_max) / max(
        1, len(roll_vals)
    )
    ...
    return {
        "terminal_nz_cmd_variance": safe_var(nz_vals),
        "terminal_roll_rate_cmd_variance": safe_var(roll_vals),
        "terminal_throttle_cmd_variance": safe_var(throttle_vals),
        "terminal_nz_limit_exceedance_rate": nz_exceed,
...
def aggregate_metrics(episodes):
    ...
    result = {
        ...
        "terminal_nz_cmd_variance": safe_mean(
            [e["terminal_nz_cmd_variance"] for e in episodes]
        ),
        "terminal_roll_rate_cmd_variance": safe_mean(
            [e["terminal_roll_rate_cmd_variance"] for e in episodes]
        ),
        "terminal_throttle_cmd_variance": safe_mean(
            [e["terminal_throttle_cmd_variance"] for e in episodes]
        ),
        "terminal_nz_limit_exceedance_rate": safe_mean(
            [e["terminal_nz_limit_exceedance_rate"] for e in episodes]
        ),
        "terminal_roll_rate_limit_exceedance_rate": safe_mean(
            [e["terminal_roll_rate_limit_exceedance_rate"] for e in episodes]
        ),
...
def run_benchmark(
    config_path, smoke=False, episodes=None, seeds=None, scenarios=None, output_dir=None
):
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        for m in all_method_metrics:
            f.write(
                ...
                f"{m.get('terminal_nz_limit_exceedance_rate', np.nan):.2%} | "
                f"{m.get('terminal_roll_rate_limit_exceedance_rate', np.nan):.2%} |\n"
            )
