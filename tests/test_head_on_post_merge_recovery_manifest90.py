...
def test_recovery_manifest90_keeps_minrange_only_post_merge_guard():
    ...
    recovery_cfg = resolved["virtual_point"]["post_merge_tactical_basis_recovery_profile"]
    assert recovery_cfg["tactical_basis_action_io_entry_lateral_sign_hold_enabled"] is True
    assert (
        recovery_cfg["tactical_basis_action_io_entry_lateral_sign_hold_release_vp_lateral_bias_m"]
        == 250.0
    )
    assert recovery_cfg["entry_window_steps"] == 8
    assert recovery_cfg["tactical_basis_action_io_entry_window_min_abs"] == 0.18
    assert (
        recovery_cfg[
    ...
    assert recovery_cfg["tactical_basis_action_ll_entry_window_floor"] == -0.05
    assert (
        recovery_cfg[
    ...
    assert recovery_cfg["tactical_basis_action_cd_entry_window_min"] == 0.30
    assert recovery_cfg["predicted_target_forward_scale_entry_window_override"] == 0.0
    assert (
        recovery_cfg[
    ...
    assert recovery_cfg["reason_overrides"] == {
        "close_range_reengagement_crossing": {
            "conditions": {
                "max_aa_deg": 129.0,
