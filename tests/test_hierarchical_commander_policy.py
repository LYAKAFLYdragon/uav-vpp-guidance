...
class _GuardEnv:
    def __init__(
        ...
        *,
        ...
        self._vp_lateral_bias_m = float(vp_lateral_bias_m)
        self._vp_forward_bias_m = float(vp_forward_bias_m)
        self._last_virtual_point = {
            "position_neu": np.array(
                [
                    self._range_m + self._vp_forward_bias_m,
                    self._vp_lateral_bias_m,
    ...
    def _get_current_states(self, noisy=False):
        del noisy
        altitude_m = float(
            self._altitude_sequence_m[
                min(self._state_call_index, len(self._altitude_sequence_m) - 1)
            ]
        )
        self._state_call_index += 1
        own = {
            "position_m": np.array([0.0, 0.0, altitude_m], dtype=np.float64),
            "velocity_vector_mps": np.array([250.0, 0.0, 0.0], dtype=np.float64),
